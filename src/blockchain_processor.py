...
class BlockchainProcessor(Processor):
    ...
    def __init__(self, config, shared):
        ...
        self.cloakcoind_url = 'http://%s:%s@%s:%s/' % (
            config.get('cloakcoind', 'cloakcoind_user'),
            config.get('cloakcoind', 'cloakcoind_password'),
            config.get('cloakcoind', 'cloakcoind_host'),
            config.get('cloakcoind', 'cloakcoind_port'))
    ...
    def do_catch_up(self):
        ...
        self.header = self.block2header(self.cloakcoind('getblock', (self.storage.last_hash,)))
        ...
        while not self.shared.stopped():
            ...
            if self.shared.paused():
                print_log("cloakcoind is responding")
                self.shared.unpause()
            time.sleep(10)
    ...
    def cloakcoind(self, method, params=[]):
        postdata = dumps({"method": method, 'params': params, 'id': 'jsonrpc'})
        while True:
            try:
                respdata = urllib.urlopen(self.cloakcoind_url, postdata).read()
            except:
                print_log("cannot reach cloakcoind...")
                self.wait_on_cloakcoind()
            else:
                r = loads(respdata)
                if r['error'] is not None:
                    if r['error'].get('code') == -28:
                        print_log("cloakcoind still warming up...")
                        self.wait_on_cloakcoind()
                        continue
    ...
    def get_header(self, height):
        block_hash = self.cloakcoind('getblockhash', [height])
        b = self.cloakcoind('getblock', [block_hash])
    ...
    def init_headers(self, db_height):
        ...
        try:
            while height < db_height:
                ...
                #print_log("vuksa2....", header)
                print_log("vuksa2HEIGHT....", height)
                #print_log("vuksaDB....................", db_height)
                if height > 1:
                    if prev_hash != '2d8251121940abce6e28df134c6432e8c5a00d59989a2451806c2778c3a06112':
                        #print_log("vuksa3....", prev_hash)
                        # The prev_hash block is orphaned, go back
                        #print_log("reorganizing, a block in file is orphaned:", prev_hash)
                        #print_log("vuksa4....", header.get('prev_block_hash'))
    ...
    def hash_header(self, header):
        #print_log("vuksa5....", rev_hex(Hash(header_to_string(header).decode('hex')).encode('hex')))
    ...
    def get_mempool_transaction(self, txid):
        try:
            raw_tx = self.cloakcoind('getrawtransaction', [txid, 0])
        except:
            return None
    ...
    def import_block(self, block, block_hash, block_height, sync, revert=False):
        ...
        # add undo info
        if not revert:
            self.storage.write_undo_info(block_height, self.cloakcoind_height, undo_info)
    ...
    def process(self, request, cache_only=False):
        ...
        if method == 'blockchain.numblocks.subscribe':
            ...
            try:
                txo = self.cloakcoind('sendrawtransaction', params)
            ...
            tx_hash = params[0]
            result = self.cloakcoind('getrawtransaction', [tx_hash, 0])

        elif method == 'blockchain.estimatefee':
            num = int(params[0])
            result = self.cloakcoind('estimatefee', [num])
    ...
    def getfullblock(self, block_hash):
        block = self.cloakcoind('getblock', [block_hash])
        ...
        try:
            ...
            logger.error("cloakcoind error (getfullblock)",exc_info=True)
            self.shared.stop()
        ...
        for ir in r:
            if ir['error'] is not None:
                self.shared.stop()
                print_log("Error: make sure you run cloakcoind with txindex=1; use -reindex if needed.")
    ...
    def catch_up(self, sync=True):
        ...
        while not self.shared.stopped():
            ...
            # are we done yet?
            info = self.cloakcoind('getinfo')
            self.cloakcoind_height = info.get('blocks')
            cloakcoind_block_hash = self.cloakcoind('getblockhash', [self.cloakcoind_height])
            if self.storage.last_hash == cloakcoind_block_hash:
            ...
            self.up_to_date = False
            try:
                next_block_hash = self.cloakcoind('getblockhash', [self.storage.height + 1])
    ...
    def memorypool_update(self):
        mempool_hashes = set(self.cloakcoind('getrawmempool'))
