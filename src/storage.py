...
class Storage(object):
    ...
    def __init__(self, config, shared, test_reorgs):
        ...
        self.db_version = 4 # increase this when database needs to be updated
        try:
    ...
    def listunspent(self, addr):
        key = self.address_to_key(addr)
        if key is None:
            raise BaseException('Invalid CloakCoin address', addr)
    ...
    def write_undo_info(self, height, cloakcoind_height, undo_info):
        if height > cloakcoind_height - 100 or self.test_reorgs:
